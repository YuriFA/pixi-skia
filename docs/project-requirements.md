...
Задача:
Создать приложение на TypeScript, которое объединяет возможности `pixi.js` и `Skia`, реализует собственную обертку для рендера Pixi-контейнера с помощью Skia и позволяет экспортировать результат в PDF с использованием Skia PDF backend.
Задания
...
- При этом результат должен быть векторным, а не просто изображением, вставленным в PDF.
...
Ожидаемый результат:
Проект загруженный на github + работающая программа запущенная на любом бесплатном хостинге + pdf файл сгенерированный с помощью skia с векторной графикой. Выполненные работы принимаются до 15-го июня включительно по ссылке https://forms.yandex.ru/u/6a0db569eb6146662ae0fb45
